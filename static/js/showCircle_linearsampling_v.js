...
for (var i = c1; i <= c2; i++) {
  ...
  $("<input/>", { type: "radio", name: "Ncol", id: "cols_" + i, onchange: "refreshByCol(" + i + "," + dispsize + ")", value: "" + i }).appendTo(
    "#setCol"
  );
...
function svg_load_draw_go(d = 40, R = 250, circleScale = 50, halfOpen = 20, Nrow = 1, Ncol = 1, inFile) {
  ...
  for (var i = 0; i < Nrow; i++) {
    ...
    $("<div/>", { id: "row_" + i }).appendTo("#svg_block");
    $("<button/>", {
      type: "button",
      id: "row_" + i + "_btn",
  ...
  $.getJSON(pairingFile, function (data, status) {
    ...
    for (var i = 0; i < Math.min(Nrow * Ncol, dispsize); i++) {
      ...
      svg_draw_graphs(svgid, data.pairing.info, data.pairing.pairs[i], d, R, circleScale, halfOpen);
...
function fillSeqText(info, struc) {
  ...
  document.getElementById("lvn0").innerHTML = "Structure # " + '<select class="select" id="copyIdx" onchange="setCopyStruc()"></select>: ';
