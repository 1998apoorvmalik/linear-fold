...
// svg_load_draw_go(d=40,R=215,circleScale=50,halfOpen=20, Nrow=3, Ncol=4, inFile=$("#path").text()); // draw graphs


var dispsize = Math.min(18, parseInt($('#samplesize').text()));
var Nrow=3, Ncol=4, c1=2, c2=6;

if (dispsize < 7){
    Nrow = 1;
    c2 = Math.min(dispsize,6);
    c1 = Math.min(2, c2);
}

$('#setCol').html('Columns on display: ')
for (var i=c1; i<=c2; i++){
// <input type="radio" name="myRadios" onchange="handleChange2();" value="2" />
    $('<input/>',{type: "radio",
                  name: "Ncol",
                  id: 'cols_'+i,
                  onchange: "refreshByCol("+i+","+dispsize+")",
                  value: ''+i
                 }).appendTo('#setCol');
    $('<span/>',{html: i+'&nbsp&nbsp'}).appendTo('#setCol');
}

function refreshByCol(Ncol, dispsize=18){
    Nrow = Math.min(3, Math.ceil(dispsize/Ncol));
    svg_load_draw_go(d=40,R=215,circleScale=50,halfOpen=20, Nrow=Nrow, Ncol=Ncol, inFile=$("#path").text()); // draw graphs
}

// refreshByCol(4, dispsize);
// $('#cols_4').click();
$('#cols_'+Math.min(4, dispsize)).click();  // if dispsize < 4


function svg_load_draw_go(d=40,R=250,circleScale=50,halfOpen=20,Nrow=1,Ncol=1,inFile){
    ...
    $('#svg_block').html('');
    // console.log('hello {0}'.format(pairingFile));

    var viewWid = parseInt(1500/Ncol);
    var gridWid = parseInt(12/Ncol);
    R = viewWid / 2 - d + 5;
    for (var i=0; i<Nrow; i++){
        var rowid = 'svg_row_'+i;
        $('<div/>', {id: 'row_'+i,
                    }).appendTo('#svg_block');
        $('<button/>',{type: "button",
                    id: 'row_'+i+'_btn',
                    // html: '<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#'
                    //     +rowid+'">Show Structure '+(i*Ncol+1)+'-'+(i*Ncol+Ncol)
                    //     +'</button>'
                    class: "btn btn-info",
                    'data-toggle': "collapse",
                    'data-target': "#"+rowid,
                    text: 'Structure '+(i*Ncol+1)+'-'+Math.min(i*Ncol+Ncol, dispsize)+": Show ▼",
                    }).appendTo('#row_'+i);
        // $('<div/>', {class: 'row collapse', id: rowid }).appendTo('#svg_block');
        $('<div/>', {class: 'row collapse', id: rowid }).appendTo('#row_'+i);

        for (var j=0; j<Ncol; j++){
            if (i*Ncol+j == dispsize) break;
            var svgid = "SVG_"+(i*Ncol+j);
            $('<div/>', {
                // html: '<svg version="1.1" id="'+svgid+'" width="700" height="600"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                html: '<svg version="1.1" id="'+svgid+'" viewBox="0 0 '+viewWid+' '+(viewWid+20)+'"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
                class: 'col-'+gridWid
            }).appendTo('#'+rowid);
    ...
    // not working if outside this function
    $('[data-toggle="collapse"]').click(function() {
      var controlled = $(this).attr("data-target");
      // $(this).toggleClass( "active" );
      // if ($(this).hasClass("active")) {
      if ($(controlled).hasClass("show")) {
    ...
    $.getJSON(pairingFile, function(data,status) {
        ...
        fillSeqText(data.pairing.info, data.pairing.strucs[0]);                                  // fill sequence and pairing structures at bottom of page
        // $("#SVG_1").empty();
        for (var i=0; i<Math.min(Nrow*Ncol, dispsize); i++){
            var svgid = "SVG_"+i;
            $("#"+svgid).empty();
            // console.log(svgid);
            // svg_draw_graphs(svgid, data.pairing,d,R,circleScale,halfOpen);
            svg_draw_graphs(svgid,
                            data.pairing.info,
                            data.pairing.pairs[i],
                            d,R,circleScale,halfOpen);
            // insertForna(data.pairing);
            // break;
        }
        $('#row_0_btn').click();//collapse('toggle');;
    });
}

function svg_draw_graphs(svgid="mySVG", info, pairs, d,R,circleScale,halfOpen=20) {
        // kaibo added 06/30/2020, for constraints
        // if pairingList.length > 16,
        //   then idx 16 is [constraint_pair_list, constraint_unpair]
        //   idx 17 is conSeq
        // svg_drawFrame(svgid,pairingList[0],d,R,circleScale,halfOpen,half="right",add_cons=(pairingList.length>16));
        // svg_fillCircles(svgid,pairingList,d,R,circleScale,halfOpen,pairingList[1]);
        // console.log(info);
        svg_drawFrame(svgid,info.len,d,R,circleScale,halfOpen,half="right",add_cons=false);
        svg_fillCircles(svgid,info,pairs,d,R,circleScale,halfOpen=20,beamsize=0)
}
...
function svg_drawFrame(svgid,N,d,R,circleScale,halfOpen=20,half="left",add_cons=false){
    ...
    //display legend for shared pairs, or constraints
        ...
        var x1 = 1.92*R, x2 = 2.7*R, y1 = 11, y2 = 90;
        if (add_cons){
    ...
    if (half == "left"){
        ...
        //var svg_ref = document.getElementById('SVG_1ref');
...
function svg_drawCircle(svgid,x0,y0,R,halfOpen=20){
    ...
    if(svg != null)
    {
        ...
        // svg.appendChild(newarc); if svg == document.getElementById()
        svg.append(newarc);
    }
}
...
function svg_fillCircles(svgid,info,pairs,d,R,circleScale,halfOpen=20,beamsize=0){
    var a = R + d;
    var b = 3.5*R + 3*d;  //2*d is also ok
    //var data = loadInfo();
    var consLst = null;
    ...
    svg_fillCircle(svgid, info.seq, info.t, [0,0,info.energy],[],pairs,[],info.len, a,a+H_title,R,halfOpen,consLst);
}
...
function svg_fillCircle(svgid,seq,t,P_R_F,missing,hit,wrong,N,x0,y0,R,halfOpen=20,consLst=null){
    ...
    var svgidx = parseInt(svgid.substring(4))+1;
    var svg = document.getElementById(svgid);
    if(svg != null)
    {
        //set font and style
        ...
        PRSize = 18;
        // var thisTitle = "Top #"+svgidx+", \u0394G= "+P_R_F[2]+" kcal/mol";
        var thisTitle = "Structure "+svgidx;
        // var newmark = getNode('text', {x: x0-94-P_R_F[2].length*4, y:y0-R-H_title, fontFamily:PRFont, fontSize:PRSize, class:"titles"});
        var newmark = getNode('text', {x: x0-thisTitle.length*4, y:y0-R-H_title, fontFamily:PRFont, fontSize:PRSize, class:"titles"});
        newmark.textContent = thisTitle;
        svg.appendChild(newmark);
    }
...
function svg_drawArc(svgid,pair,n1,n2,N,x0,y0,R,color,halfOpen=20){
    ...
    arcid = pair+', d='+(n2-n1)+', ['+(n1+1)+', '+(n2+1)+'], #'+svgidx;
    // console.log(arcid);
    ...
    if(svg != null)
    {
        ...
        if (arc){
            ...
            var newline = getNode('line', {x1: p1.x, y1:p1.y, x2: p2.x, y2:p2.y, stroke:color, strokeWidth:arcWidth, class:"tooltips-arcs arcs"+color+dashClass, id:arcid});  // class:"arcs"
...
function fillSeqText(info, struc){
        ...
        document.getElementById("dlBtnTitle1").innerHTML = 'LinearSampling (sample size '+info.samplesize+')';
        document.getElementById("nameEcho").innerHTML = info.name +' (len:'+info.len+')';
        document.getElementById("beamEcho").innerHTML = info.beam;
        //// document.getElementById("timeLCEcho").innerHTML = data[2];
        document.getElementById("timeLVEcho").innerHTML = info.time;//(''+data[3]).slice(0,-5);
        document.getElementById("energyEcho").innerHTML = info.energy;//(''+data[3]).slice(0,-5);

        document.getElementById("seq0").innerHTML = 'seq: ';//<br>';
        //// document.getElementById("lcf0").innerHTML = 'LinearFold-C: ';//<br>';
        // document.getElementById("lvn0").innerHTML = 'Structure # 1: ';//<br>';
        document.getElementById("lvn0").innerHTML = 'Structure # '+
                                                    '<select class="select" id="copyIdx" onchange="setCopyStruc()"></select>: ';
        for (var i=1;i<=dispsize;i++){
           $('<option/>',{value: i+'', text: i+''}).appendTo('#copyIdx');
        }
        if (dispsize < parseInt($('#samplesize').text())){
            $('#lvn-copy-area-addon').html('More structures can be found in download file');
        }

        document.getElementById("seq").innerHTML = info.seq;
        //// document.getElementById("lcf").innerHTML = data[6];
        document.getElementById("lvn").innerHTML = struc;
        ...
        var textWid = 100, t = '';
        var conSeq = null;
        // if (data.length>16) {conSeq = data[17];}
        document.getElementById("seq-show").innerHTML = linebreak(info.seq,textWid);
        //// document.getElementById("lcf-show").innerHTML = linebreak(data[6],textWid);
        document.getElementById("lvn-show").innerHTML = linebreak(struc,textWid, conSeq);
...
function setCopyStruc(){
    var pairingFile = "/pairingRes/"+$("#path").text();
    var idxStr = $("#copyIdx").find('option:selected').text();
    if (idxStr == 'more in file'){ return; }
    var idx = parseInt(idxStr)-1;
    $.getJSON(pairingFile, function(data,status) {
        struc = data.pairing.strucs[idx];
        document.getElementById("lvn").innerHTML = struc;
        document.getElementById("lvn-show").innerHTML = linebreak(struc, 100, null);
        $("#btn_copy_lvn").text('Copy to clipboard');
    });
}
