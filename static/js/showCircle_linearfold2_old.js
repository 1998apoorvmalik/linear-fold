...
function svg_drawFrame(svgid,N,d,R,circleScale,halfOpen=20,half="left",add_cons=false){
    ...
    //display legend for shared pairs
        ...
        if (add_cons){
            var legend = getNode('text', {x: 1.96*R, y:100, fontFamily:legendFont, fontSize:18, fill:'blue'});
...
function svg_fillCircles(svgid,data,d,R,circleScale,halfOpen=20,beamsize=0){
    ...
    if (data.length > 16){
        consLst = data[16];
        //console.log(consLst[0], consLst[1]);
    }
    svg_fillCircle(svgid,data[5],data[2],data[8],data[9],data[10],data[11],N,a,a+H_title,R,halfOpen,consLst);
    //fill circle bottom-right with linearvn_missing, linearvn_hit, linearvn_wrong
    svg_fillCircle(svgid,data[5],data[3],data[12],data[13],data[14],data[15],N,b,a+H_title,R,halfOpen,consLst);
}
...
function svg_drawNucleotide(svgid,N,dotLst,seq,x0,y0,R,halfOpen=20){
    ...
    if(svg != null)
    {
        ...
        for (var i = 0; i < dotLst.length; i++){
            ...
            //var newdot = getNode('circle', {cx:p.x, cy:p.y, r:radius,strokewidth:radius,class:'nt_'+seq[i]+' tooltips-arcs',  id:ntId});//,      stroke:color_dot_stroke, fill:color_dot_fill, strokewidth:5}); //color defined in css;// later: add id and class
...
function fillSeqText(data){
        ...
        document.getElementById("lcf-show").innerHTML = linebreak(data[6],textWid, conSeq);
