...
svg_load_draw_go(svgid="mySVG_MEA", d=40,R=250,circleScale=50,halfOpen=20, inFile=$("#path").text().replace('.res','.mea.res')); // draw graphs
svg_load_draw_go(svgid="mySVG", d=40,R=250,circleScale=50,halfOpen=20, inFile=$("#path").text()); // draw graphs

//svg_go(d=40,R=250,circleScale=50,halfOpen=20);
function svg_load_draw_go(svgid="mySVG", d=40,R=250,circleScale=50,halfOpen=20, inFile){
    ...
    $.getJSON(pairingFile, function(data,status) {
        ...
        fillSeqText(svgid, data.pairing);   // fill sequence and pairing structures at bottom of page

        $("#"+svgid).empty();
        svg_draw_graphs(svgid, data.pairing, d,R,circleScale,halfOpen);
        //insertForna(data.pairing);     // not used in linearpartition
    });
}

function svg_draw_graphs(svgid, pairingList, d,R,circleScale,halfOpen=20) {
        svg_drawFrame(svgid,pairingList[0],d,R,circleScale,halfOpen,half="right");
...
function svg_drawFrame(svgid,N,d,R,circleScale,halfOpen=20,half="left"){
    ...
    if (half == "left"){
        ...
        //var newtext = getNode('text', {x: R-2.7*d,   y:d-0.5*H_title, fontFamily:titleFont, fontSize:titleSize, class:"titles"});//LinearPartition-C
        var titleIdx = 5;
        if (svgid == 'mySVG_MEA') titleIdx += 2;
        var title = titles[titleIdx];
        var newtext = getNode('text', {x: a-title.length*9,   y:d-0.5*H_title, fontFamily:titleFont, fontSize:titleSize, class:"titles"});//LinearPartition-C
        newtext.textContent = title; //titles[2];
        svg.appendChild(newtext);

        var title = titles[titleIdx+1];
        //var newtext = getNode('text', {x:b-4*d, y: d-0.5*H_title, fontFamily:titleFont, fontSize:titleSize, class:"titles"});//LinearPartition-V
        var newtext = getNode('text', {x: b-title.length*9,   y:d-0.5*H_title, fontFamily:titleFont, fontSize:titleSize, class:"titles"});//LinearPartition-C
...
function svg_fillCircles(svgid,data,d,R,circleScale,halfOpen=20,beamsize=0){
    ...
    svg_fillCircle(svgid,data[2][0],data[8],data[9],data[10],data[11],N,a,a+H_title,R,halfOpen);
    //fill circle bottom-right with linearvn_missing, linearvn_hit, linearvn_wrong
    svg_fillCircle(svgid,data[3][0],data[12],data[13],data[14],data[15],N,b,a+H_title,R,halfOpen);
    
    //// //fill the only circle (LinearPartition-V) with linearvn_missing, linearvn_hit, linearvn_wrong
    //// svg_fillCircle(svgid,data[3],data[12],data[13],data[14],data[15],N,a,a+H_title,R,halfOpen);
}
...
function svg_fillCircle(svgid,t,P_R_F,missing,hit,wrong,N,x0,y0,R,halfOpen=20){
    ...
    for (var i=0; i<missing_pair; i+=3){
        //svg_drawArc(svgid,missing[i],missing[i+1],N,x0,y0,R,'LightGray',halfOpen); 
        svg_drawArc(svgid,missing[i],missing[i+1],missing[i+2],N,x0,y0,R,'green',halfOpen); 
    }

    var hit_pair = hit.length;
    for (var i=0; i<hit_pair; i+=3){
        svg_drawArc(svgid,hit[i],hit[i+1],hit[i+2],N,x0,y0,R,'rgb(0, 0, 255, '+parseFloat(hit[i+2])+')',halfOpen);
    }

    var wrong_pair = wrong.length;
    for (var i=0; i<wrong_pair; i+=3){
        //svg_drawArc(wrong[i],wrong[i+1],N,x0,y0,R,'red',halfOpen);
        svg_drawArc(svgid,wrong[i],wrong[i+1],wrong[i+2],N,x0,y0,R,'red',halfOpen);
    }

    //get corresponding svg object
    
    var svg = document.getElementById(svgid);
    if(svg != null && svgid != 'mySVG_MEA')
    {  
        //set font and style
        ...
        if (x0 < 400){
             ...
             var newmark = getNode('text', {x: x0-88-P_R_F[2].length*4, y:y0-R-H_title/2, fontFamily:PRFont, fontSize:PRSize, class:"titles"});
             newmark.textContent = "time="+t+"s,  Score="+P_R_F[2];
             //var newmark = getNode('text', {x: x0-88-P_R_F[2].length*4, y:y0-R-H_title/2, fontFamily:PRFont, fontSize:PRSize, class:"titles"});
             //console.log('hehe '+P_R_F[2].length);
...
function svg_drawArc(svgid,n1,n2,p,N,x0,y0,R,color,halfOpen=20){
    // Kaibo 3/14/2019: from clockwise to counterclockwise (5' left and 3' right)
    ...
    {  
        ...
        if (arc){
            ...
            arcid = '['+(n1+1)+', '+(n2+1)+'], p='+roundToTwo(parseFloat(p)); // kaibo update on 02/02/2020: not `n1+1` and `n2+1`, since Vincent's output is 1-based, different from LinearFold
            var colortext = color;
            if (color.includes('rgb')) { colortext = 'blue'; }
            var attr = {d: pathstr, stroke:color, fill:"none", strokeWidth:arcWidth, class:"tooltips-arcs arcs"+colortext, id:arcid};   //instead of fill:"transparent"
            var newarc = getNode('path', attr);
            svg.appendChild(newarc);
        } else {
...
var H_title = 20;
var titles = ["CONTRAfold MFE", "Vienna RNAfold", "LinearPartition-C", "LinearPartition", "Gold/Ref", "LinearPartition-C", "LinearPartition-V", "LinearPartition-C MEA", "LinearPartition-V MEA"];
//var titles = ["CONTRAfold MFE", "Vienna RNAfold", "LinearFold-C", "LinearFold-V", "Gold/Ref"];

function fillSeqText(svgid, data){
    ...
    if (svgid != 'mySVG_MEA') {
        ...
        document.getElementById("timeLCEcho").innerHTML = data[2][0];//(''+data[3]).slice(0,-5);
        document.getElementById("time2LCEcho").innerHTML = data[2][1];// kaibo added 06/30/2020, special for partition, two time
        document.getElementById("timeLVEcho").innerHTML = data[3][0];//(''+data[3]).slice(0,-5);
        document.getElementById("time2LVEcho").innerHTML = data[3][1];// kaibo added 02/01/2020, special for partition, two time
    } else {
        document.getElementById("seq0").innerHTML = 'seq: ';//<br>';
        document.getElementById("lcf0").innerHTML = 'LinearFold-C MEA: ';//<br>';
        document.getElementById("lvn0").innerHTML = 'LinearFold-V MEA: ';//<br>';
