FORNA_URL = "https://forna-server.herokuapp.com/";
...
function loadPreset() {
  ...
  fetch(
    "https://raw.githubusercontent.com/LinearFold/LinearTurboFold/main/sars-cov-2_results/sars-cov-2_and_sars-related_25_genomes_msa_structures.txt"
  )
...
for (sCounter in modifiedDataArray) {
  ...
  //
  holder = [];
  for (sCount in modifiedDataArray[sCounter].struc) {
    //sCount is the index
...
function shrink(start, end, whichSeq) {
  ...
  if (sequences == null) {
    ...
    while (leftCounter <= end) {
      if (modifiedDataArray[whichSeq].struc[leftCounter - 1] == ")") {
        if (tempArray[leftCounter] < start) {
          // if its pair is out of index
...
function updateSequences() {
  ...
  while (posStart < firstLength) {
    ...
    for (num in sequences) {
      ...
      returnString +=
        selectThis.options[sequences[num]].innerHTML.padStart(14, ",").fontcolor("blue") + (posStart + 1).toString().padStart(6, ",") + " ";
...
function applyRange25() {
  ...
  for (item_ in sequences) {
    ...
    addition +=
      '<iframe id="Evanforna' +
      item +
      `" src="${FORNA_URL}?id=fasta&amp;file=>header%5Cn` +
      seqFornaNew +
      "%5Cn" +
      strucFornaNew +
    ...
    for (dictCount in modifiedDataArray) {
      ...
      while (nucleoCounter < holdingSeq.length) {
        newString +=
          ...
          dictCount +
          "_" +
          (counting + nucleoCounter + 1) +
          "_" +
          pairingArray[dictCount].pairs[counting + nucleoCounter + 1] +
          ">" +
          holdingSeq[nucleoCounter] +
          "</span>";
        nucleoCounter += 1;
      }
      newString += "&nbsp;" + endPoint;
      ...
      while (sequenceCounter < holdingStruc.length) {
        newString +=
          ...
          dictCount +
          "_" +
          (counting + sequenceCounter + 1) +
          "_" +
          pairingArray[dictCount].pairs[counting + sequenceCounter + 1] +
          ">" +
          holdingStruc[sequenceCounter] +
          "</span>";
        sequenceCounter += 1;
      }
      newString += "&nbsp;" + endPoint;
  ...
  while (posStart < lengthCounter) {
    ...
    for (dCount in myData) {
      ...
      for (nCounter in holderSeq) {
        ...
        //end
        finalString +=
          '<span class ="test" id=Seq' + dCount + "_" + (parseInt(nCounter) + posStart + 1) + "_" + matched + ">" + holderSeq[nCounter] + "</span>";
      }
      finalString += " " + posEnd;
      finalString += "<br>";
      ...
      for (iCounter in holderStruc) {
        ...
        //end
        finalString +=
          '<span class ="test" id=Stru' +
          dCount +
          "_" +
          (parseInt(iCounter) + posStart + 1) +
          "_" +
          sMatched +
          ">" +
          holderStruc[iCounter] +
          "</span>";
      }
      finalString += " " + posEnd;
      finalString += "<br>";
...
function linebreak(data, textWid, tup, triple = false) {
  ...
  var t = "",
    i;
  for (i = 0; i < lst.length - textWid; i += textWid) t = t + lst.slice(i, i + textWid).join("") + " <br>";
