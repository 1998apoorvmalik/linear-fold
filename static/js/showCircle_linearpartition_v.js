...
function svg_drawFrame(svgid,N,d,R,circleScale,halfOpen=20,half="left"){
    ...
    if (half == "left"){
        ...
        var newtext = getNode('text', {x: a-title.length*9,   y:d-0.5*H_title, fontFamily:titleFont, fontSize:titleSize, class:"titles"});//LinearPartition-V

        //var newtext = getNode('text', {x: R-2.2*d,   y:d-0.5*H_title, fontFamily:titleFont, fontSize:titleSize, class:"titles"});//LinearFold-C
...
function fillSeqText(svgid, data){
    ...
    if (svgid != 'mySVG_MEA') {
        ...
        document.getElementById("time2LVEcho").innerHTML = data[2];// kaibo added 02/01/2020, special for partition, two time
    } else {
