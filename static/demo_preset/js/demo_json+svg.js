...
function svg_drawNucleotide(svgid,N,seq,x0,y0,R,halfOpen=20){
    ...
    {  
        ...
        for (var i = 0; i < N; i++){
            ...
            var newdot = getNode('circle', {cx:p.x, cy:p.y, r:radius,strokewidth:radius,class:'nt_'+seq[i]});//, stroke:color_dot_stroke, fill:color_dot_fill, strokewidth:5}); //color defined in css;// later: add id and class
...
function svg_drawArc(svgid,seq,n1,n2,N,x0,y0,R,color,halfOpen=20){
    ...
    {  
        ...
        if (arc){
            ...
            var colorInit=color[0].toUpperCase();;
            if (colorInit == 'L') colorInit = 'G';
            arcid = seq[n1]+seq[n2]+' ['+colorInit+'], d='+(n2-n1)+', ['+(n1+1)+', '+(n2+1)+']';
