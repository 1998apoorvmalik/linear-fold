...
def my_form_LS():
    if request.method == 'POST':
        url = "http://10.217.112.122:7001/linear-sankoff"
...
def showRes_LS_v(name, samplesize):
    return flask.render_template('showResult_linearsampling_v.html', pairingRes=name+'.sampling.res', samplesize=samplesize)
...
def inputSeq_LV():
    if flask.request.method == 'POST':
        ...
        if 'seqFile' in flask.request.files and flask.request.files['seqFile'].filename != u'':
...
def LP_v_core(filename, seq, seqName, beamsize, T0, usrIP):
    ...
    for line in lpv:
        ...
        if i > j: i, j = j, i
        lst += [i, j, float(a[2])]
        if (i-1, j-1) in lpv_mea_pairs: lst_v_mea += [i, j, float(a[2])]
    ...
    os.system('chmod 644 '+pairingFile)
    logInfo = "[{0}] [len: {1:0>6}] [time: {2:0>12.5f}s] [LP file: {3}] [IP: {4}]".format(time.asctime(), len(seq) , T2, filename, usrIP)
    addlog(logInfo)
        ...
        if input_flag:
            ...
            #try:
                ...
                for line in lpc:
                    ...
                    if i > j: i, j = j, i
                    lst_c += [i, j, float(a[2])]
                    if (i-1, j-1) in lpc_mea_pairs: lst_c_mea += [i, j, float(a[2])]

                #write results of lpv and lpc to a final partition.res result (json)
                pairingFile = pairingDir + filename + '.partition.res'   #output with pairingName
                #arc_pairing_single_json.LoadSave_lpv(pairingFile,seq,lst,t2,t3,beamsize,seqName,score2)
                arc_pairing_single_json.LoadSave_lp2(pairingFile,seq,lst_c,tc2,tc3,beamsize,seqName,score_c,lst_v,tv2,tv3,score_v)
                pairingFile_mea = pairingDir + filename + '.partition.mea.res'   #output with pairingName
                arc_pairing_single_json.LoadSave_lp2(pairingFile_mea,seq,lst_c_mea,-1,-1,beamsize,seqName,score_c,lst_v_mea,-1,-1,score_v,lpc_mea, lpv_mea)

                T2 = time.time() - T0

                os.system('chmod 644 '+pairingFile)
                logInfo = "[{0}] [len: {1:0>6}] [time: {2:0>12.5f}s] [LP2 file: {3}] [IP: {4}]".format(time.asctime(), len(seq) , T2, filename, usrIP)
                addlog(logInfo)
...
def inputSeq():
    if flask.request.method == 'POST':
        ...
        if is_constraint: is_zuker = False  # only either constraint or zuker allowed
        print('zuker:', is_zuker)
        if is_zuker:
            zuker_delta = flask.request.form['zukerDelta']
            try:
                if not zuker_delta or float(zuker_delta) > 10: zuker_delta = '5'
            except:
        ...
        if input_flag:
            ...
            if LF_v_only:
                # if True:
                try:
                    newurl = LF_v_core(filename, seq, seqName, beamsize, T0, usrIP, conSeq, ' with Zuker suboptimal structures: ' if is_zuker else ': ')
                    return flask.redirect(newurl)
                except:
                    return "Sorry, Something wrong dealing with the sequence during LinearFold-V only mode"
            try:
                #if True:
                t1, t2 = request_ironcreek(filename)    # call lc/lv functions in ironcreek, save results in files, return time
                #print(filename)
                ...
                print('[time {}/{}s] [score {}/{}] [b {}] [LF file {}]'.format(t1,t2,score1,score2,beamsize,filename))
                #write results of lc, lv to a final pairing.res result
                pairingFile = pairingDir + filename + '.pairing.res'   #output with pairingName
                arc_pairing_single_json.LoadSave(pairingFile,seq,lc[line_idx_res][:-1],lv[line_idx_res][:-1],t1,t2,beamsize,seqName,score1,score2,conSeq)   # Vincent version
                ...
                '''
                os.system('chmod 644 '+pairingFile)
                logInfo = "[{0}] [len: {1:0>6}] [time: {2:0>12.5f}s] [LF2 file: {3}] [IP: {4}]".format(time.asctime(), len(seq) , T1, filename, usrIP)
                addlog(logInfo)
                #global pairingName#, total_time         # Kaibo remove 3/11/2019
                #pairingName = filename+'.pairing.res'   # Kaibo remove 3/11/2019

                newurl = flask.url_for('showRes_old',name=filename, dld_cmt=' with Zuker suboptimal structures: ' if is_zuker else ': ')
                #print(newurl)
                #show(newurl)
                return flask.redirect(newurl)
            except:
...
def LS_v_core(filename, seq, seqName, beamsize, samplesize, T0, usrIP):
    ...
    if t == 'wrong' or t.startswith('error'): return 'wrong'
    t = round(float(t),2)
    ...
    arc_pairing_single_json.LoadSave_ls(pairingFile,seq,outLSv,t,beamsize,seqName,samplesize,nDisp=min(18,samplesize))

    T2 = time.time() - T0

    os.system('chmod 644 '+pairingFile)
    logInfo = "[{0}] [len: {1:0>6}] [time: {2:0>12.5f}s] [LS file: {3}] [IP: {4}]".format(time.asctime(), len(seq) , T2, filename, usrIP)
    addlog(logInfo)
...
def postprocessInputSeq(lines, is_constraint=False):
    ...
    wrong_symb = ''.join(re.findall('[^AUCGTNaucgtn]+',seq))
    if len(seq) == 0 or len(wrong_symb) > 0: return None, seq, None, False
    seq = seq.upper()
    seq = seq.replace("T", "U")
...
def inputSeq_LTF(): #purpose- create an output file to do what?
    ...
    assert outDirLTF1 == ironcreekOutDir_LTF+jobID, 'output to {}, not {}'.format(outDirLTF1, ironcreekOutDir_LTF+jobID)
