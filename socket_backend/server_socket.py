...
from dirs import usrDir, tmpDir, outDir, create_dirs
...
while True:
    ...
    time_s = time.time()
    os.system(
        "/scratch/dengde/fastdecode/fastcky_rerun/build_beamckypar/beamckypar -f {} -b {} > {}".format(
            tmpSeqFile, beamsize, outLc
        )
    )  ## linearfold-c
    ...
    time_s = time.time()
    os.system(
        "{}; /scratch/dengde/fastdecode/fastcky_vienna/fastcky/build/beamckypar -f {} -b {} > {}".format(
            LD, tmpSeqFile, beamsize, outLv
        )
    )  ## linearfold-v
    ...
    c.send(
        bytes(
            "output to %s" % (outPairPath)
            + "\n>> linearFold-C\n"
            + lc[-1]
            + "total process time: %f" % (time1)
            + "\n>> linearFold-V\n"
            + lv[-1]
            + "total process time: %f" % (time2),
            encoding="utf8",
        )
    )  # +\
    ...
    c.send(
        bytes(
            "[{0}] [len: {1:0>7}] [time: {2:0>12.5f}s] [file: {3}]".format(
                time.asctime(), len(seq), time1 + time2, inFileName
            ),
            encoding="utf8",
        )
    )
